...
function PrintPesananSukses($Order) {
	$BatasBayar = new DateTime($Order['DateEstimate']);
	...
	<div class="contact100-status">
		...
		<div class="clearfix">
			<p>
				...
				<br><b>Batas waktu pembayaran : </b><b style="color: red">{$BatasBayar->format('l, d F Y H:i')}</b>
			</p>
			<br>
			<table>
				<tr>
...
function PrintTrackingPesanan($DB, $Order) {
	$Estimasi = new DateTime($Order['DateEstimate']);
	...
	$SQL = "SELECT COUNT(*) FROM `DP_ORDER` WHERE `Status` > 0 AND `Status` < 3 AND `DateEstimate` < '{$Order['DateEstimate']}'";
	//echo $SQL;
	$Stmt = $DB->query($SQL);
	$Data = $Stmt->fetch();
	$Count = 1;
	...
	$Queue = ($Order['Status'] != 5) ? "<h6>Estimasi Selesai : <div style=\"color: red\">{$Estimasi->format('l, d F Y H:i')}</div>Antrian ke - {$Count}</h6>" : "<h5>Pesanan selesai, Terima Kasih :)</h5>";
